...
public class DisplayFinanceDetails {

	JFrame frame;
	private JTable tbpat;
	String head2[] ={ "AssetId ","Bank","Types of Finance","Bank Ref","Original Loan Amount","Date of loan","Maturity Date" };
	...
	public static void main(String[] args) {
   		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					DisplayFinanceDetails window = new DisplayFinanceDetails();
	...
	public void showRecords()
	{
		...
		try
		{
			createCon();	//call method createCon()  
                 
			Statement st = con.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,
			ResultSet.CONCUR_UPDATABLE);
			ResultSet r1;
			r1 = st.executeQuery("select * from asset_finance_table");
